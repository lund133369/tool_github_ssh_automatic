...
def preguntar_alias(proyectos):
    ...
    while True:
        ...
        if alias in existentes:
            ...
            console.print('[red]Alias inválido. Usa solo letras, números, guiones.[/red]')

def generar_clave(alias):
    key_path = os.path.join(SSH_DIR, f'id_rsa_{alias}')
    if os.path.exists(key_path):
        sobre = inquirer.confirm(message=f'La clave {key_path} ya existe. ¿Sobrescribir?', default=False).execute()
        if not sobre:
            return key_path
        os.remove(key_path)
...
def agregar_a_ssh_agent(key_path):
    try:
        ...
        if 'Error connecting to agent' in stderr:
            console.print('[yellow]ssh-agent no está activo. Puedes iniciarlo manualmente si lo deseas.[/yellow]')
        else:
            console.print(f'[yellow]No se pudo agregar la clave al ssh-agent: {stderr}[/yellow]')
...
def esperar_confirmacion(repo_url):
    ...
    if m:
        ...
        console.print(f'[yellow]Abriendo la página de llaves SSH del repositorio:[/yellow] {url_settings}')
        webbrowser.open(url_settings)
    else:
        console.print('[red]No se pudo extraer usuario y repo de la URL. Abre manualmente la configuración de llaves SSH en GitHub.[/red]')
...
def pedir_repo_url():
    while True:
        url = inquirer.text(message='URL SSH de GitHub (git@github.com:user/repo.git):').execute()
...
def seleccionar_carpeta():
    ...
    try:
        ...
        if salida:
            ...
            return salida
        print('[red]No seleccionaste ninguna carpeta. Intenta de nuevo o ingresa la ruta manualmente.[/red]')
        return inquirer.text(message='Pega aquí la ruta de la carpeta de destino:').execute()
    except Exception as e:
...
def clonar_repo(alias, url, destino):
    try:
        console.print(f'[cyan]Clonando el repositorio {url} en {destino} con alias {alias}...[/cyan]')
        if not os.path.exists(destino):
            os.makedirs(destino)
        ruta = os.path.join(destino, alias)
        key_path = os.path.normpath(os.path.expanduser(os.path.join(SSH_DIR, f'id_rsa_{alias}')))
        subprocess.run([
            'git', 'clone', url, ruta,
            '--config', f'core.sshCommand=ssh -i "{key_path}"'
        ], check=True)
        return ruta
    except Exception as e:
...
def validar_prerequisitos():
    ...
    for lib in ['rich', 'InquirerPy', 'pyperclip']:
        if importlib.util.find_spec(lib) is None:
            errores.append(f'Falta la librería Python: {lib}. Instala con: pip install {lib}')
    # Tkinter
    try:
        import tkinter
    except ImportError:
        errores.append('Falta la librería tkinter. Instálala según tu sistema operativo.')
    if errores:
